...
import { modalState, moviesSate } from '../atom/ModalAtom'
...
const Modal = () => {
    const [movie, setMovie] = useRecoilState(moviesSate)
    ...
    return (
        <MuiModal open={showModal} onClose={handleClose} className='fixed !top-7 left-0 right-0 mx-auto w-full max-w-5xl overflow-hidden overflow-y-scroll rounded-md  scrollbar-hide'>
            <>
                <div >
                    <button onClick={handleClose} className=' modalButton absolute  right-5 top-5 !z-40  h-9 w-9  border-none  bg-[#181818]'><XIcon className='w-6 h-6' /></button>
                </div>
                ...
                <div className='relative pt-[56.25%]'>
                    ...
                    <div className='absolute bottom-10 flex items-center justify-between px-10 w-full'>
                        <div className=' flex space-x-2'>
                            ...
                            <button className='modalButton'><PlusIcon className='h-7 w-7' /></button>
                            <button className='modalButton'><ThumbUpIcon className='h-7 w-7' /></button>
                ...
                <div className='flex space-x-16  rounded-b-md bg-[#181818] px-10 py-8'>
                    <div className='space-y-6 text-lg'>
                        <div className='flex items-center space-x-2 text-sm '>
                            ...
                            <div className='flex h-4 items-center justify-center border border-white/40 px-1.5 text-xs'>HD</div>
                        </div>
                        <div className='flex flex-col  gap-x-10 gap-y-4  font-light md:flex-row'><p className='w-5/6'>{movie?.overview}</p>
                            <div className='flex flex-col sapce-y-3 text-sm'>
                                <div><span className='text-[gray]'>Genres: </span>
                                    {genres.map((genre) => genre.name).join(',')}
                                </div>
                                <div> <span className='text-[gray] '>Original Language: </span>
